...
import { IconMail, IconLock, IconUser, CrownIcon } from '../components/Icons'

function CastleSVG() {
  return (
    <svg viewBox="0 0 400 350" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: 110, height: 'auto', filter: 'drop-shadow(0 4px 20px rgba(212,168,67,0.18))', animation: 'float 5s ease-in-out infinite' }}>
...
export default function SignupPage() {
  ...
  const fields = [
    { key: 'username', label: 'USERNAME', icon: IconUser, type: 'text', ph: 'Your name' },
    { key: 'email', label: 'EMAIL', icon: IconMail, type: 'email', ph: 'you@example.com' },
    { key: 'password', label: 'PASSWORD', icon: IconLock, type: 'password', ph: 'Create a password' },
    { key: 'confirm', label: 'CONFIRM PASSWORD', icon: IconLock, type: 'password', ph: 'Confirm your password' },
  ]
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-card anim-fade-up">
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 2 }}><Link to="/" style={{ display: 'inline-flex' }}><CastleSVG /></Link></div>
            ...
            return (
              <div key={f.key} className="auth-field">
                ...
                <div style={{ position: 'relative' }}>
                  <Icon size={15} color="var(--text-muted)" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
                  <input className="input" style={{ padding: '9px 14px 9px 42px' }} type={f.type} placeholder={f.ph} value={form[f.key]} onChange={u(f.key)} />
                </div>
              </div>
            )
          })}
          <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '11px 0', fontSize: '0.84rem', marginTop: 2 }}>
            Sign Up
          </button>
        </form>
        <div className="divider" />
        <p style={{ fontSize: '0.8rem', textAlign: 'center', color: 'var(--text-muted)' }}>
          Already a noble? <Link to="/login" style={{ color: 'var(--gold)', fontWeight: 600 }}>Enter the gates →</Link>
        </p>
      </div>
      ...
      <style>{`
        ...
        .auth-title { font-family:var(--font-brand); font-size:1.35rem; color:var(--gold); text-align:center; margin-bottom:2px; }
        .auth-sub { color:var(--text-muted); font-size:0.76rem; text-align:center; margin-bottom:14px; }
        .auth-form { display:flex; flex-direction:column; gap:8px; }
        .auth-field { display:flex; flex-direction:column; gap:3px; }
        .auth-label { font-size:0.65rem; color:var(--text-secondary); font-weight:700; letter-spacing:0.05em; }
      `}</style>
    </div>
  )
}
